...
export default function MemoryPage() {
  ...
  const handleAdd = async () => {
    ...
    await window.electronAPI.memory.create({ content: newContent.trim() })
    setNewContent('')
    loadEntries()
  }
  ...
  const injectMemory = () => {
    ...
    return entries.map(e => `[记忆: ${e.content}]`).join('\n')
  }
    ...
    <div className="flex-1 overflow-y-auto" style={{ backgroundColor: 'var(--bg-primary)' }}>
      <div className="max-w-2xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>🧠 记忆系统</h1>
        ...
        <div className="space-y-2">
          {entries.length === 0 && (
            <div className="text-center py-12 text-sm" style={{ color: 'var(--text-muted)' }}>还没有记忆，添加一条试试</div>
          )}
          {entries.map((entry) => (
